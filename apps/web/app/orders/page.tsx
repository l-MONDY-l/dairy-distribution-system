...
export default function OrdersPage() {
  ...
  // If search narrows down to a single shop, auto-select it for convenience
  useEffect(() => {
  ...
  const loadData = async () => {
    try {
      ...
      const [ordersData, shopsData, agentsData, productsData, driversData, batchesData] =
        await Promise.all([
          getOrders(),
          getShops(),
          getAgents(),
      ...
      prev.map((item, i) =>
        i === index
          ? {
              ...item,
              [field]:
                field === 'qty' ? Number(value) || 1 : value,
            }
          : item,
      ),
    );
  ...
  const handleCancel = async (order: Order) => {
    try {
      ...
      setSuccess(`Order ${order.orderNo} cancelled.`);
      await loadData();
    } catch (err: any) {
  ...
  const handleReject = async (order: Order) => {
    try {
      ...
      setSuccess(`Order ${order.orderNo} rejected.`);
      await loadData();
    } catch (err: any) {
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold">Agent Orders</h1>
            <p className="mt-1 text-sm text-slate-400">
              Manage agent orders by shop, agent, and region. Create new orders
              via the button.
            </p>
          </div>
          <button
        ...
        {/* Top status cards similar to Stock page */}
          ...
          <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  Filter by pending, approved, or on-hold orders.
                </p>
              </div>
                ...
                <table className="min-w-full overflow-hidden rounded-2xl border border-slate-800">
                  <thead className="bg-slate-950">
                    <tr className="text-left text-sm text-slate-400">
                      ...
                      <th className="px-4 py-3 font-medium">Agent price</th>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 px-4 py-6" onClick={(e) => { if (e.target === e.currentTarget && !saving) setShowCreateModal(false); }}>
          <div className="w-full max-w-3xl rounded-3xl border border-slate-700 bg-slate-900 p-6 shadow-xl">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-lg font-semibold text-slate-100">Create Order</h3>
              <button
                type="button"
            ...
            <form onSubmit={handleCreateOrder} className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                {shopId && (
                  <div className="rounded-2xl border border-slate-700 bg-slate-950 px-4 py-2 text-xs text-slate-300">
                    {(() => {
                      ...
                      return `Shop city: ${cityName || '—'} • Town: ${townName || '—'}`;
                    })()}
                  </div>
                )}
                ...
                {agentId && shopId && (
                  <div className="rounded-2xl border border-slate-700 bg-slate-950 px-4 py-2 text-xs text-slate-300">
                    {(() => {
                      ...
                      const cityName = (s as any)?.city?.name ?? '';
                      const townName = (s as any)?.town?.name ?? '';
                      return `Agent city: ${cityName || '—'} • Town: ${townName || '—'}`;
                    })()}
                  </div>
                )}
                ...
                {driverId && shopId && (
                  <div className="rounded-2xl border border-slate-700 bg-slate-950 px-4 py-2 text-xs text-slate-300">
                    {(() => {
                      ...
                      const cityName = (s as any)?.city?.name ?? '';
                      const townName = (s as any)?.town?.name ?? '';
                      return `Driver city: ${cityName || '—'} • Town: ${townName || '—'}`;
                    })()}
                  </div>
                )}
              ...
              <div className="space-y-3">
                ...
                {items.map((item, index) => {
                  ...
                  const productBatches = stockBatches.filter((b) => {
                    ...
                    return exp >= today; // hide expired stock
                  });
